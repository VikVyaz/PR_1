...
def test_cashback_profit(mock_data: MagicMock, mock_category: MagicMock) -> None:
...
def test_to_filter_data(fixt_cashback: list) -> None:
    ...
    assert to_filter_data(fixt_cashback[0], fixt_cashback[1], fixt_cashback[2]) == [fixt_cashback[0][0]]


def test_to_get_category(fixt_cashback: list, fixt_expected_get_category: dict) -> None:
